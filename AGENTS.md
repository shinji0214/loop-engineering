...
- **System Prompt**（要旨。実体は `index.js` の `DEVELOPER_SYSTEM`）:
  ...
  - 既存コードが提示されている場合（FROM_DIR）は変更するファイルだけ出力。
    新規タスクは初回に全ファイル、2回目以降は変更したファイルだけ出力（出さないファイルは保持）
  - 削除は ```delete ブロックにパスを1行ずつ
  - パスに .. や絶対パスは使わない
  ```
- **既存コード入力**: `FROM_DIR=<path>` で既存のファイルツリーを `currentFiles` の初期値にできる
  （新規生成ではなく改修・自己編集タスク向け）。`node_modules`/`.git`/生成ディレクトリ等は自動除外。
...
`test/`（`npm test`）は、ループを自分自身の改修に使うときの外部の歯止め。

- `test/mocks/*.js`: `index.js` の挙動を検証する**手書き**統合テスト（オフラインモック）。ループには生成させない
- `test/generations/gen-NNN/`: 承認された世代が生成した受け入れテスト。世代ごとに追加され、以降の全世代を縛る
- 生成物は `LOOP_ARTIFACT_ROOT` で隔離するのでテスト実行が本体を汚さない
- 詳細は [test/README.md](./test/README.md)
